...
const DropFileZone = ({ loading, setLoading, formData, onFormDataChange, id, extensions, handleDropSubmit, type }) => {

    const { acceptedFiles, getRootProps, getInputProps } = useDropzone({
        accept: {
            'application/x-gtf': ['.gtf'],
            'application/x-fasta': extensions,
        },
        multiple: true,
        onDrop: async (files) => {
            if (loading) {
            ...
            if (files.length > 2) {
                toast.info('You can only upload a maximum of 2 files.');
                return;
            }
            ...
            try {
                await handleDropSubmit(files, type);
                ...
                console.error('Error in handleDropSubmit:', err);
            } finally {
                setLoading(false);
            }
        },
